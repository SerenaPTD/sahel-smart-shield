...
bool sss_sensors_init()
{
    ...
    {
        if (DEBUG_MODE)
            Serial.println("[ERREUR] SHT31 non détecté (adresse 0x44)");
        tout_ok = false;
    }
    else
    {
        ...
        if (DEBUG_MODE)
            Serial.println("[OK]     SHT31 initialisé");
    }
    ...
    {
        if (DEBUG_MODE)
            Serial.println("[ERREUR] DS18B20 non détecté (GPIO4) — vérifier résistance 4.7kΩ");
        tout_ok = false;
    }
    else
    {
        ...
        if (DEBUG_MODE)
            Serial.printf("[OK]     DS18B20 initialisé (%d capteur détecté)\n",
    ...
    {
        if (DEBUG_MODE)
            Serial.println("[ERREUR] INA219 non détecté (adresse 0x40)");
        tout_ok = false;
    }
    else
    {
        ...
        ina219.setBusRange(INA219_BRNG_16); // Plage bus 16V (suffisant pour 12V)
        if (DEBUG_MODE)
            Serial.println("[OK]     INA219 initialisé");
    }
    ...
    if (DEBUG_MODE)
        Serial.println("[OK]     ZMPT101B prêt (ADC GPIO34)");
...
void lire_sht31(SSS_Mesures &m)
{
    ...
    {
        if (DEBUG_MODE)
            Serial.println("[ERREUR] Lecture SHT31 échouée — valeur NaN ou hors plage");
        m.sht31_ok = false;
...
void lire_ds18b20(SSS_Mesures &m)
{
    ...
    while (!ds18b20.isConversionComplete())
    {
        ...
        {
            if (DEBUG_MODE)
                Serial.println("[ERREUR] DS18B20 timeout conversion");
            m.ds18b20_ok = false;
            return;
        }
    }
    ...
    {
        if (DEBUG_MODE)
            Serial.println("[ERREUR] DS18B20 déconnecté ou hors plage");
        m.ds18b20_ok = false;
    }
}
...
void lire_ina219(SSS_Mesures &m)
{
    ...
    {
        if (DEBUG_MODE)
            Serial.printf("[ERREUR] INA219 tension hors plage : %.2f V\n", tension);
        m.ina219_ok = false;
    }
}
